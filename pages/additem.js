...
const AddItem = (props) => {
	...
	useEffect(() => {
		setValue("seller", props.uid);
	}, []);

	const handleAddItem = async (filledData) => {
		let formData = new FormData();

		formData.append("product", JSON.stringify(filledData));
		acceptedFiles.forEach((file) => {
			formData.append("productImages", file);
		});

		toast.info("Please wait while we are uploading your data");
		setIsLoading(true);
